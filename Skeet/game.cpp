...
void Game :: advance()
{
   ...
   handleCollisions();
   // cleans up the dead bird or used bullet
   cleanUpZombies();
}
...
Bird* Game :: createBird()
{
   ...
   // If the random number is 1, it points to the Standard class
   if (rand == 1)
   {
	   newBird = new Standard;
   }

   // If the random number is 2, it points to the ToughBird Class
   else if (rand == 2)
   {
	   newBird = new ToughBird;
   }

   // Else, it points to the SacredBird Class
   else if (rand == 3)
   {
      ...
      if (bullets[i].isAlive())
      {
         ...
         if (bird != NULL && bird->isAlive())
         {
                ...
                && fabs(bullets[i].getPoint().getY() - bird->getPoint().getY()) < CLOSE_ENOUGH)
            {
			   ...
			   // if the bird is dead, reset the bird pointer to NULL
			   if (!bird->isAlive())
			       bird = NULL;
            }
         }
